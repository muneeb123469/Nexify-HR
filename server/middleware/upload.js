...
const validateFileType = (file) => {
  ...
  if (!allowedMimeTypes.includes(file.mimetype)) {
    return {
      isValid: false,
      error: `Invalid MIME type '${file.mimetype}'. File may be corrupted or not a valid document.`
    };
  }
...
const enhancedUpload = (req, res, next) => {
  ...
  singleUpload(req, res, (err) => {
    ...
    if (req.file) {
      try {
        // Read file buffer for corruption detection
        const filePath = req.file.path;
        const buffer = fs.readFileSync(filePath);
        
        const corruptionCheck = detectFileCorruption(buffer, req.file.originalname);
