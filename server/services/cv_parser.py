...
def guess_name(text):
    ...
    name_patterns = [
        ...
        r'full\s*name\s*:\s*([A-Z][a-z]+ [A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)',
...
def extract_skills(text):
    ...
    # Method 3: Programming language and framework patterns
    tech_patterns = [
        r'\b(JavaScript|TypeScript|Python|Java|C\+\+|C#|PHP|Ruby|Go|Rust|Swift|Kotlin)\b',
        r'\b(React|Angular|Vue|Node\.js|Express|Django|Flask|Spring|Laravel)\b',
        r'\b(MySQL|PostgreSQL|MongoDB|Redis|Elasticsearch|Docker|Kubernetes)\b',
        r'\b(AWS|Azure|GCP|Git|Jenkins|CI/CD|REST|GraphQL|API)\b'
    ]
    ...
    while i < len(lines):
        ...
        if is_institution_line(current_line):
            ...
            while j < len(lines) and lines_processed < max_lookahead:
                ...
                # If we hit another institution, stop processing for the current entry
                if is_institution_line(next_line):
                    break

                # Check for degree/field on the same line
                degree, field = extract_degree_info(next_line)
                if degree and entry["degree"] is None:
                    entry["degree"] = degree
                    logger.debug(f"Found degree in line {j}: '{degree}'")
                if field and entry["field"] is None:
                    entry["field"] = field
                    logger.debug(f"Found field in line {j}: '{field}'")
                
                j += 1
                lines_processed += 1

            if entry["institution"] and (entry["degree"] or entry["field"]):
                items.append(entry)
                logger.debug(f"Added education entry: {entry}")
